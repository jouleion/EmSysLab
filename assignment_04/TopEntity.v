module TopEntity(
    input       clk,
	input 	 	btn1,		// as a manual reset line.
    input       PITCH_DIRA,  // Pin assignment for dir[0]
    input       PITCH_DIRB,  // Pin assignment for dir[1]
    output reg  led1,		// clockwise direction indication
    output reg  led2		// counterclockwise direction indication
);

// State of state machine, bits correspond to A and B dir.
parameter S00 = 2'b00;
parameter S01 = 2'b01;
parameter S10 = 2'b10;
parameter S11 = 2'b11;

parameter steps_per_revolution = 1;

// Current and previous state
reg [1:0] current_state = S00;
reg [1:0] previous_state = S00;

// Combine PITCH_DIRA and PITCH_DIRB into a 2-bit vector
wire [1:0] dir;
assign dir = {PITCH_DIRA, PITCH_DIRB};

// Counter for steps and display
reg [7:0] steps = 0; 
reg [7:0] previous_steps = 0;

// Sequential logic: state update and counter/LEDs on clock edge
always @(posedge clk) begin

	if(btn1) begin
		led1 <= 0;
		led2 <= 0;
		current_state <= S00;
		previous_state <= S00;
		steps <= 0;
		previous_steps <= 0;
	end

	// Save previous state for edge detection
	previous_state <= current_state;
	previous_steps <= steps;

	// Compute next state (combinational logic folded into sequential)
	case (current_state)
		S00: begin
			if (dir == S10) begin
				current_state <= S10;
				steps <= steps + 1;
			end else if (dir == S01) begin
				current_state <= S01;
				steps <= steps - 1;
			end else begin
				current_state <= S00;
			end
		end
		S01: begin
			if (dir == S11) begin 
				current_state <= S11;
				steps <= steps + 1;
			end else if (dir == S00) begin
				current_state <= S00;
				steps <= steps - 1;
			end else begin
				current_state <= S01;
			end
		end
		S10: begin
			if (dir == S00) begin
				current_state <= S00;
				steps <= steps - 1;
			end else if (dir == S11) begin
				current_state <= S11;
				steps <= steps + 1;
			end else begin
				current_state <= S10;
			end
		end
		S11: begin
			if (dir == S01) begin
				current_state <= S01;
				steps <= steps + 1;
			end else if (dir == S10) begin
				current_state <= S10;
				steps <= steps - 1;
			end else begin
				current_state <= S11;
			end
		end
		default: current_state <= S00;
	endcase

	// Assign an LED to indicate the direction of rotation.
	if(previous_steps < steps) begin
		// Clockwise
		// led1 <= 1; 
		// led2 <= 0;
	end else if (previous_steps > steps) begin
		// Counterclockwise
		// led1 <= 0;
		// led2 <= 1; 
	end

	if(steps / steps_per_revolution >= 1) begin
		led1 <= 1; // Turn on LED1 for one revolution
		led2 <= 0;
	end else if (steps / steps_per_revolution <= -1) begin
		led1 <= 0;
		led2 <= 1; // Turn on LED2 for one revolution in the opposite direction
	end else begin
		led1 <= 0;
		led2 <= 0; // Both LEDs off when less than one revolution
	end
end
endmodule